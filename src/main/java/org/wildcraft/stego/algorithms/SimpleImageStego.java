package org.wildcraft.stego.algorithms;

import org.springframework.stereotype.*;
import org.wildcraft.coverfile.algorithm.CoverRepository;
import org.wildcraft.framework.CoverRepoInterface;
import org.wildcraft.framework.SteganoAlgorithmInterface;
...
public class SimpleImageStego implements SteganoAlgorithmInterface {

    public File hideData(File file, String data) {
        String filePath = getPath(file.getAbsolutePath(), file.getName());
        String [] fileNameWithExtn = getFileNameAndExtn(file.getName());
        String fileName = fileNameWithExtn[0];
        String fileExtn = fileNameWithExtn[1];

        encode(filePath, fileName,fileExtn, fileName+"Stegan",data);

        return new File(filePath+fileName+"Stegan."+fileExtn);
    }
    ...
    public String unHideDate(File file) {
        ...
        return decode(filePath,fileName);
    }

    public static void main(String[] args) {
        CoverRepoInterface coverRepoInterface = new CoverRepository();
        File imageFile = coverRepoInterface.getCoverFile("DummyData");
        SimpleImageStego simpleImageStego = new SimpleImageStego();
        simpleImageStego.hideData(imageFile,"Hello I am Secret Data");

        String filePath = getPath(imageFile.getAbsolutePath(), imageFile.getName());
        String [] fileNameWithExtn = getFileNameAndExtn(imageFile.getName());
        String fileName = fileNameWithExtn[0];
        String fileExtn = fileNameWithExtn[1];

        String message = simpleImageStego.unHideDate(new File(filePath+fileName+"Stegan."+fileExtn));
        System.out.println(message);

    }

    private static String getPath(String absolutePath, String fileName) {
        return absolutePath.replaceAll(fileName,"");
    }

    private static String [] getFileNameAndExtn(String fileNameWithExtn) {
        String [] file = fileNameWithExtn.split("\\.(?=[^\\.]+$)");
        return file;
    }
         ...
         */
        private boolean encode(String path, String original, String ext1,
                              String stegan, String message) {
            String fileName = getFullImagePath(path, original, ext1);
            BufferedImage originalImage = getBufferedImage(fileName);

            // user space is not necessary for Encrypting
            BufferedImage image = getUserSpace(originalImage);
            image = hideTextIntoImage(image, message);

            return (saveImageFile(image, new File(getFullImagePath(path, stegan, "png")),
                    "png"));
        }
         ...
         * @param type integer representing either basic or advanced encoding
         */
        private String decode(String path, String name) {
            byte[] decode;
            try {
                // user space is necessary for decrypting
                BufferedImage image = getUserSpace(getBufferedImage(getFullImagePath(path, name,
                        "png")));
         ...
         */
        private String getFullImagePath(String path, String name, String ext) {
         ...
         */
        private BufferedImage getBufferedImage(String f) {
         ...
         */
        private boolean saveImageFile(BufferedImage image, File file, String ext) {
         ...
         */
        private BufferedImage hideTextIntoImage(BufferedImage image, String text) {
        ...
        private BufferedImage getUserSpace(BufferedImage image) {
            // create newImage with the attributes of image
